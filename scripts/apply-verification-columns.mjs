...
const statements = [
  ...
  `ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "verification_token_hash" TEXT`,
  `ALTER TABLE "User" ADD COLUMN IF NOT EXISTS "verification_token_expires_at" TIMESTAMP(3)`,
...
async function apply(label, databaseUrl) {
  ...
  try {
    ...
    `;
    console.log(`[${label}] OK — ${host} — email_verified_at: ${cols.length > 0 ? "yes" : "NO"}`);
  } catch (e) {
